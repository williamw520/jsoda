...
** TODO Support select count(*) in Query, for SimpleDB.
** TODO Support select count(*) in Query, for DynamoDB.
** TODO Add unit test for select count(*) for different queries.
